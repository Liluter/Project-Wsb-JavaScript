...
function actWeatherDest(where) {
  fetch("http://api.openweathermap.org/data/2.5/weather?lat=".concat(airportData.airport[where].lat, "&lon=").concat(airportData.airport[where]["long"], "&appid=e88cbba2770d843c73cbfc59c2d7a3b0&lang=pl&units=metric")).then(function (response) {
    ...
    if (where == "wroclaw") {
      navWeatherStatus.textContent = "".concat(currentTime, " | ").concat(actWeatherData.name, " | ").concat(actWeatherData.weather[0].description, " | ").concat(actWeatherData.main.temp.toFixed(1), " \u2103 | ").concat(actWeatherData.wind.speed, " km/h");
...
arrivalDate.setAttribute("min", calTomorrow); // departureDate.setAttribute("value", calCurrentDate);
...
var zapiszData = function zapiszData(data, where) {
  airportData[where] = data; // console.log("Sprawdzenie odczyt: ", airportData.where, airportData);
...
acceptArrivalDateBtn.addEventListener("click", function () {
  console.log("Data Arrival accepted", arrivalDate.value); // airportData = Object.assign(airportData, {"trip": { "arrivaldate": arrivalDate.value}}); //czyścoi obiekt

  airportData.trip.arrivaldate = arrivalDate.value; // airportData.trip.arrivaldate = arrivalDate.value;
}, false);
...
/******/ })()
;
//# sourceMappingURL=index.723ecde6b3962ff1e7ec.bundle.js.map